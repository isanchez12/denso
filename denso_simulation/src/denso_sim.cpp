...
using namespace denso_simulation;

DENSOSim::DENSOSim(const std::string &prefix,
    hardware_interface::JointStateInterface &js_interface,
    hardware_interface::EffortJointInterface &ej_interface) :
  //Not taking into account the pincher for this version
  n_dof_(6),
  n_actuated_dof_(6),
  initialized_(false),
  prefix_(prefix),
    ...
    std::vector<gazebo_ros_control::JointData> joints)
{
  ...
  // Actuated joints
  joint_name_[0] = prefix_ + "denso_J0";
  joint_name_[1] = prefix_ + "denso_J1";
  joint_name_[2] = prefix_ + "denso_J2";
  joint_name_[3] = prefix_ + "denso_J3";
  joint_name_[4] = prefix_ + "denso_J4";
  joint_name_[5] = prefix_ + "denso_J5";
...
void DENSOSim::readSim(ros::Time time, ros::Duration period)
{
...
void DENSOSim::writeSim(ros::Time time, ros::Duration period)
{
  if(!initialized_) { return; }
